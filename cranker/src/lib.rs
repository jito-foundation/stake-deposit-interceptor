...
impl InterceptorCranker {
    ...
    async fn process_expired_receipts(&self) -> Result<(), CrankerError> {
        ...
        for (receipt_pubkey, receipt) in receipts {
            ...
            debug!(
                "Receipt {} raw bytes:\n\
                 ...
                 cool_down: {}\n\
                 current_time: {}",
                receipt_pubkey, deposit_time, cool_down, now
            );
                ...
                Some(expiry_time) => {
                    if now > expiry_time {
                        info!(
                            ...
                            receipt_pubkey, now, expiry_time
                        );
                        match self.claim_pool_tokens(&receipt).await {
                            Ok(_) => {
                                info!("Successfully claimed tokens for receipt {}", receipt_pubkey);
    ...
    async fn get_deposit_receipts(&self) -> Result<Vec<(Pubkey, DepositReceipt)>, CrankerError> {
